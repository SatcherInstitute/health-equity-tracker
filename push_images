...
docker build -t gcr.io/$PROJECT_ID/dedupe-service -f run_dedupe/Dockerfile .
...
docker push gcr.io/$PROJECT_ID/dedupe-service
...
DEDUPE_DIGEST=$(gcloud container images describe gcr.io/$PROJECT_ID/dedupe-service --format="value(image_summary.digest)")
...
echo "dedupe_image_digest=\"$DEDUPE_DIGEST\"" >> config/digest.tfvars
