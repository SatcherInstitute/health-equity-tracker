...
def create(
    ...
    - using --from-source-environment option
      ...
      Example:
      ```
      composer-dev create --from-source-environment remote_composer_env --location us-central1 --project
      project_id --port 8081 --dags-path my_project/dags local_env_name
      ```
    - using --from-image-version option
      (based on a given Composer image version)\n
      Example:
      ```
      composer-dev create --from-image-version composer-2.0.17-airflow-2.2.5 --project project_id
      --port 8081 --dags-path my_project/dags local_env_name
      ```
