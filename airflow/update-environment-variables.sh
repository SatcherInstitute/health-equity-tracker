...
CHECK_PERCENT_SHARES=$(gcloud run services list --platform managed --filter check-percent-share-service --format 'value(status.url)')
echo "Using Fetched Data: ${CHECK_PERCENT_SHARES}"
...
--update-env-variables=AIRFLOW_VAR_CHECK_PERCENT_SHARES_ENDPOINT=${CHECK_PERCENT_SHARES} \
