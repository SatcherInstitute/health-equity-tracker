...
def service_request(url: str, data: dict):
    ...
    if environment == 'local':
        # Obtain the identity token for dev environment using gcloud command
        ...
        token_response = requests.get(token_request_url, headers=token_request_headers, timeout=300)
    ...
    try:
        resp = requests.post(url, json=data, headers=receiving_service_headers, timeout=300)
