...
TEST_DIR = os.path.join(os.getcwd(), "python", "tests", "data", "sanity_checks")
...
def testGenerateNationalDatasetSex():
    df = pd.read_json(CDC_RESTRICTED["sex_national"], dtype={"state_fips": str})
...
def testGenerateCountyDatasetSexError():
    ...
    expected_output = [False, {"table": "cdc_restricted_sex_county", "fips": ["06123"]}]
