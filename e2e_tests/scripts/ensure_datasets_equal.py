...
parser.add_argument("-tp", "--testproject",
                    help="GCP project name with the test data")
parser.add_argument("-d", "--dataset",
                    help="Big query dataset id name to compare")
...
def main():
    ...
    test_tables = bq_client.list_tables('%s.%s' % (test_project, dataset))

    for table in test_tables:
        test_table_name = "{}.{}.{}".format(
            table.project, table.dataset_id, table.table_id)
        expected_table_name = "{}.{}.{}".format(
            expected_project, table.dataset_id, table.table_id)

        print('checking %s against %s' %
              (test_table_name, expected_table_name))
        ...
        if 'ingestion_ts' in test_df.columns:
            test_df = test_df.drop(
                columns=['ingestion_ts']).reset_index(drop=True)

        test_df = test_df.sort_values(by=sort_values).reset_index(drop=True)

        expected_df = bq_client.query(
            query_string_expected).result().to_dataframe()
