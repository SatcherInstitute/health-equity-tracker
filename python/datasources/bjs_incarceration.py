...
def generate_raw_breakdown(demo, geo_level, table_list):
    ...
    if demo == std_col.SEX_COL:
        ...
        df_jail[Sex.MALE] = df_jail[RAW_JAIL_COL].astype(float) * df_jail["Male Pct"] / 100
        df_jail[Sex.FEMALE] = df_jail[RAW_JAIL_COL].astype(float) * df_jail["Female Pct"] / 100
    ...
    if demo == std_col.RACE_OR_HISPANIC_COL:
        ...
        for race in STANDARD_RACE_CODES:
            if race in bjs_races and race != "ALL":
                df_jail[race] = df_jail[race].astype(float) * df_jail[Race.ALL.value].astype(float) / 100
    ...
    if geo_level == STATE_LEVEL:
        ...
        if demo == std_col.SEX_COL or demo == std_col.AGE_COL:
            df_prison[std_col.ALL_VALUE] = df_prison[std_col.ALL_VALUE].combine_first(df_prison[Race.ALL.value])
...
def generate_raw_national_age_breakdown(table_list):
    ...
    df_prison[RAW_PRISON_COL] = df_prison[PRISON_PCT_SHARE_COL] * total_raw_prison_value / 100
...
def post_process(df, breakdown, geo, children_tables):
    ...
    df = generate_per_100k_col(df, RAW_PRISON_COL, std_col.POPULATION_COL, PRISON_PER_100K_COL)
    df = generate_per_100k_col(df, RAW_JAIL_COL, std_col.POPULATION_COL, JAIL_PER_100K_COL)

    raw_to_share_cols_map = {RAW_PRISON_COL: PRISON_PCT_SHARE_COL, RAW_JAIL_COL: JAIL_PCT_SHARE_COL}

    if breakdown == std_col.RACE_OR_HISPANIC_COL:
        # some states and all territories will have unknown race data
        df = generate_pct_share_col_with_unknowns(
            df, raw_to_share_cols_map, std_col.RACE_CATEGORY_ID_COL, all_val, Race.UNKNOWN.value
        )
    else:
    ...
    df = df.rename(
        columns={
            std_col.POPULATION_PCT_COL: std_col.INCARCERATION_POP_PCT_SHARE,
            std_col.POPULATION_COL: std_col.INCARCERATION_POP_RAW,
        }
    )
...
class BJSIncarcerationData(DataSource):
    ...
    def write_to_bq(self, dataset, gcs_bucket, write_local_instead_of_bq=False, **attrs):
        ...
        prisoners_app_2 = standardize_appendix_table_2_df(prison_tables[APPENDIX_PRISON_2])
