...
from ingestion.merge_utils import merge_county_names, merge_state_ids, merge_pop_numbers
from ingestion.het_types import SEX_RACE_AGE_TYPE
...
def merge_svi_data(df):
    ...
    df = df[[std_col.COUNTY_FIPS_COL, std_col.POPULATION_COL]]
    df = pd.merge(svi_df, df, how="outer", on=[std_col.COUNTY_FIPS_COL])
