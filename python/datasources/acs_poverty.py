...
from ingestion.standardized_columns import (STATE_FIPS_COL, COUNTY_FIPS_COL,
                                            STATE_NAME_COL, COUNTY_NAME_COL, INCOME_COL, SEX_COL,
                                            AGE_COL, ABOVE_POVERTY_COL, BELOW_POVERTY_COL,
...
class AcsPovertyIngestor:
    ...
    def __init__(self, base_url):
        ...
        self.metadata = parseMetadata(
            metadata, [MetadataKey.AGE, MetadataKey.SEX, MetadataKey.RACE], self.metadataInitializer)
    ...
    def split_data_frames(self):
        ...
        for data, population in self.data.items():
            ...
            if county_fip is None:
                state_data.append(
                    [state_fip, self.state_fips[state_fip], race, age, sex, above, below])
            else:
                county_data.append([state_fip, self.state_fips[state_fip], county_fip, self.county_fips[(
                    state_fip, county_fip)], race, age, sex, above, below])

        # Build Panda DataFrames with standardized cols
        self.poverty_by_race_age_sex_state_frame = pd.DataFrame(state_data, columns=[
            STATE_FIPS_COL,
            STATE_NAME_COL,
            RACE_COL,
            AGE_COL,
        ...
        self.poverty_by_race_age_sex_county_frame = pd.DataFrame(county_data, columns=[
            STATE_FIPS_COL,
            STATE_NAME_COL,
            COUNTY_FIPS_COL,
            COUNTY_NAME_COL,
