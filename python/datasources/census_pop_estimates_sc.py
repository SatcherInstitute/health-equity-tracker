...
sex_map = {0: "All", 1: "Male", 2: "Female"}

year_map = {"POPESTIMATE2020": "2020", "POPESTIMATE2021": "2021"}
...
class CensusPopEstimatesSC(DataSource):
    ...
    def upload_to_gcs(self, _, **attrs):
        raise NotImplementedError("upload_to_gcs should not be called for CensusPopEstimatesSC")

    def write_to_bq(self, dataset, gcs_bucket, **attrs):
        df = gcs_to_bq_util.load_csv_as_df_from_web(BASE_POPULATION_URL, dtype={"STATE": str}, encoding="ISO-8859-1")

        for geo in [STATE_LEVEL, NATIONAL_LEVEL]:
            for breakdown in [std_col.SEX_COL, std_col.RACE_CATEGORY_ID_COL]:

                breakdown_df = generate_pop_data_18plus(df, breakdown, geo)
                ...
                gcs_to_bq_util.add_df_to_bq(breakdown_df, dataset, f"multi_{demo}_age_{geo}", column_types=col_types)
...
def generate_pop_data_18plus(df, breakdown, geo):
    ...
    df = df[
        [std_col.STATE_FIPS_COL, std_col.STATE_NAME_COL, std_col.TIME_PERIOD_COL, std_col.POPULATION_COL, breakdown]
    ]

    # combine all year/state/group rows into, summing the populations
    df = (
        df.groupby([std_col.STATE_FIPS_COL, std_col.STATE_NAME_COL, std_col.TIME_PERIOD_COL, breakdown])[
    ...
    if breakdown == std_col.RACE_CATEGORY_ID_COL:
        df_alls = df[[std_col.STATE_FIPS_COL, std_col.STATE_NAME_COL, std_col.TIME_PERIOD_COL, std_col.POPULATION_COL]]
        df_alls = (
            df_alls.groupby(
                [
                    std_col.STATE_FIPS_COL,
    ...
    if geo == NATIONAL_LEVEL:
        # drop state cols
        df = df[[std_col.TIME_PERIOD_COL, std_col.POPULATION_COL, breakdown]]
