...
from datasources.bjs_prisoners_tables_utils import (standardize_table_2_df,
                                                    ...
                                                    JAIL_PCT_SHARE_COL, JAIL_PER_100K_COL,
                                                    APPENDIX_TABLE_2,
                                                    BJS_PRISONERS_ZIP,
                                                    BJS_CENSUS_OF_JAILS_ZIP,
                                                    BJS_CENSUS_OF_JAILS_CROPS, BJS_PRISONERS_CROPS,
                                                    TABLE_2,
                                                    TABLE_10,
                                                    TABLE_13,
                                                    TABLE_23,
...
def generate_raw_race_or_sex_breakdown(demo, geo_level, source_tables):
    ...
    and columns for | RAW# jail | RAW# prison | "race" or "age" or "sex" | "state_name"
...
def generate_raw_national_age_breakdown(source_tables):
    ...
    with rows for each combo of place + demographic group,
    and columns for | RAW# PRISON | RAW# JAIL | "age" group | "state_name" (national total)
...
def generate_raw_state_age_breakdown(source_tables):
    ...
    [table_2, table_13, table_23, jail_6] = source_tables

    df_jail = jail_6.rename(
        columns={RAW_JAIL_COL: std_col.ALL_VALUE})
    ...
    # df_prison with RAW COUNT Under 18 / PLACE
    df_prison = cols_to_rows(df_prison, [std_col.ALL_VALUE, "0-17"],
                             std_col.AGE_COL, RAW_PRISON_COL)
...
def post_process(df, breakdown, geo):
    ...
    if breakdown == std_col.RACE_OR_HISPANIC_COL and geo == NATIONAL_LEVEL:
        ...
        df = dataset_utils.generate_pct_share_col_without_unknowns(
            df,
            {RAW_PRISON_COL:
                PRISON_PCT_SHARE_COL, RAW_JAIL_COL: JAIL_PCT_SHARE_COL},
            breakdown,
            std_col.ALL_VALUE,
        )
...
class BJSData(DataSource):
    ...
    def write_to_bq(self, dataset, gcs_bucket, **attrs):
        ...
        table_lookup = {
            f'{std_col.AGE_COL}_{NATIONAL_LEVEL}': [prisoners_10, prisoners_13, jail_6],
            f'{std_col.AGE_COL}_{STATE_LEVEL}': [prisoners_2, prisoners_13, prisoners_23, jail_6],
