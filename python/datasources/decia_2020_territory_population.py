...
from ingestion.het_types import DEMOGRAPHIC_TYPE
...
SEX_CODES_TO_STD = {"DP1_0001": std_col.ALL_VALUE, "DP1_0049": Sex.FEMALE, "DP1_0025": Sex.MALE}
...
COMBO_RACES_SUM_MAP = {
    ...
    (
        ...
        std_col.Race.OTHER_NONSTANDARD.value,
    ): std_col.Race.MULTI_OR_OTHER_STANDARD.value,
    (
        std_col.Race.MULTI_NH.value,
        std_col.Race.OTHER_STANDARD_NH.value,
        std_col.Race.OTHER_NONSTANDARD_NH.value,
...
class Decia2020TerritoryPopulationData(DataSource):
    ...
    def generate_breakdown_df(
        ...
        if breakdown == std_col.SEX_COL:
            count_group_cols_map = get_melt_map(SEX_CODES_TO_STD, TMP_COUNT_SUFFIX)
            pct_share_group_cols_map = get_melt_map(SEX_CODES_TO_STD, TMP_PCT_SHARE_SUFFIX)
        if breakdown == std_col.RACE_OR_HISPANIC_COL:
            # TODO: fix this
            # pylint: disable=undefined-loop-variable
            race_map: Dict[Any, str] = {**RACE_CODES_TO_STD[postal], **NON_NH_TO_NH_RACE_MAP, **COMBO_RACES_SUM_MAP}
            count_group_cols_map = get_melt_map(race_map, TMP_COUNT_SUFFIX)
            pct_share_group_cols_map = get_melt_map(race_map, TMP_PCT_SHARE_SUFFIX)

        if breakdown == std_col.AGE_COL:
            df = generate_summed_age_cols(df)
            count_group_cols_map = get_melt_map(AGE_CODES_TO_STD, TMP_COUNT_SUFFIX)
            pct_share_group_cols_map = get_melt_map(AGE_CODES_TO_STD, TMP_PCT_SHARE_SUFFIX)
            # extend the melt maps to include summed age bucket cols
            count_group_cols_map = {**count_group_cols_map, **get_melt_map(STD_AGES_SUM_MAP, TMP_COUNT_SUFFIX)}
        ...
        df = dataset_utils.melt_to_het_style_df(
            ...
            [geo_col],
            {std_col.POPULATION_COL: count_group_cols_map, std_col.POPULATION_PCT_COL: pct_share_group_cols_map},
        )
...
def load_source_dfs() -> Dict[str, pd.DataFrame]:
    ...
    details."""
    return {
        postal: gcs_to_bq_util.load_csv_as_df_from_data_dir("decia_2020_territory_population", filename).drop([0])
        for postal, filename in ISLAND_SOURCE_FILE_MAP.items()
    }


def get_source_col_names(source_codes_map: Dict[str, str], metric: Literal["_count", "_pct_share"]) -> List[str]:
...
def get_value_cols(code_map: Dict):
    ...
    return get_source_col_names(code_map, TMP_COUNT_SUFFIX) + get_source_col_names(code_map, TMP_PCT_SHARE_SUFFIX)
