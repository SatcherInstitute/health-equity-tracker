...
def test_write_to_bq_black_men_by_urbanicity_national(
    ...
    datasource.write_to_bq("dataset", "gcs_bucket", demographic="urbanicity", geographic=NATIONAL_LEVEL)
    ...
    assert_frame_equal(
        actual_current_df,
        load_golden_df(GOLDEN_DIR, "black_men_by_urbanicity_national_current", DTYPE),
        check_like=True,
    )
    assert_frame_equal(
        actual_historical_df,
        load_golden_df(GOLDEN_DIR, "black_men_by_urbanicity_national_historical", DTYPE)
        .sort_values(by=["time_period", "urbanicity"])
...
def test_write_to_bq_black_men_by_urbanicity_state(
    ...
    datasource = CDCWisqarsBlackMenData()
    datasource.write_to_bq("dataset", "gcs_bucket", demographic="urbanicity", geographic=STATE_LEVEL)
    ...
    assert_frame_equal(
        actual_current_df, load_golden_df(GOLDEN_DIR, "black_men_by_urbanicity_state_current", DTYPE), check_like=True
    )
    assert_frame_equal(
        actual_historical_df,
        load_golden_df(GOLDEN_DIR, "black_men_by_urbanicity_state_historical", DTYPE)
        .sort_values(by=["time_period", "state_name", "urbanicity"])
        .reset_index(drop=True),
        check_like=True,
    )
...
def test_write_to_bq_black_men_by_age_national(
    ...
    assert_frame_equal(
        actual_current_df, load_golden_df(GOLDEN_DIR, "black_men_by_age_national_current", DTYPE), check_like=True
    )
    assert_frame_equal(
        actual_historical_df,
        load_golden_df(GOLDEN_DIR, "black_men_by_age_national_historical", DTYPE)
        .sort_values(by=["time_period", "age"])
        .reset_index(drop=True),
        check_like=True,
    )
...
def test_write_to_bq_black_men_by_age_state(
    ...
    datasource = CDCWisqarsBlackMenData()
    datasource.write_to_bq("dataset", "gcs_bucket", demographic="age", geographic=STATE_LEVEL)
    ...
    assert_frame_equal(
        actual_current_df, load_golden_df(GOLDEN_DIR, "black_men_by_age_state_current", DTYPE), check_like=True
    )
    assert_frame_equal(
        actual_historical_df,
        load_golden_df(GOLDEN_DIR, "black_men_by_age_state_historical", DTYPE)
        .sort_values(by=["time_period", "state_name", "age"])
        .reset_index(drop=True),
        check_like=True,
    )
