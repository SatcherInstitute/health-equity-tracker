...
def test_write_to_bq_alls_county(mock_csv_data_dir: mock.MagicMock, mock_bq: mock.MagicMock):
    ...
    assert_frame_equal(actual_current_df, load_golden_df(GOLDEN_DIR, current_table_name, EXP_DTYPE), check_like=True)
    ...
    assert_frame_equal(
        actual_historical_df, load_golden_df(GOLDEN_DIR, historical_table_name, EXP_DTYPE), check_like=True
    )
