...
def testPctSharesSumTo100():
    ...
    dtype['race'] = str
    dtype['race_includes_hispanic'] = object
    dtype['race_category_id'] = str

    df = pd.read_json(
        GOLDEN_DATA_RACE, dtype=dtype)

    #  remove ALL row
    df.drop(df.loc[df['race_category_id'] == "ALL"].index, inplace=True)

    pct_share_cols = [
        col for col in df.columns if "_pct_share" in col]

    for pct_share_col in pct_share_cols:
        print(pct_share_col, df[pct_share_col].sum())

    # print(pct_share_cols)
    # print(df.to_string())
