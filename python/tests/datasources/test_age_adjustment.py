...
GOLDEN_INTEGRATION_DATA = os.path.join(TEST_DIR, 'cdc_restricted-by_race_state-with_age_adjust.json')
...
def testExpectedDeaths():
    covid_data = get_cdc_restricted_by_race_age_state_as_df()
...
@mock.patch('ingestion.gcs_to_bq_util.add_dataframe_to_bq',
            return_value=None)
def testWriteToBq(mock_bq: mock.MagicMock, mock_df: mock.MagicMock):
    mock_df.side_effect = [
    ...
    expected_df = pd.read_json(GOLDEN_INTEGRATION_DATA, dtype={
        'state_fips': str,
    })
