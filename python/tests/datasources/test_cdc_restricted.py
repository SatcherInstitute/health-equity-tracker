...
def get_population_by_sex_national_as_df():
    return pd.read_csv(os.path.join(TEST_DIR, 'population_by_sex_national.csv'), dtype={
        'state_fips': str,
    })
...
def testGenerateBreakdownSexNational(mock_fips: mock.MagicMock, mock_pop: mock.MagicMock):
    ...
    df = cdc_restricted.generate_breakdown(get_cdc_restricted_by_sex_county_as_df(), 'sex', 'national')
    expected_df = pd.read_json(GOLDEN_DATA_BY_SEX_COUNTY, dtype={
        'state_fips': str,
        'county_fips': str,
        'covid_cases_share': float,
