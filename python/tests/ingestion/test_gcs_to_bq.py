...
def test_get_bq_column_types():
    ...
    column_types = gcs_to_bq_util.get_bq_column_types(fake_df, ["some_condition_per_100k"])
    expected_column_types = {"state_fips": BQ_STRING, "some_condition_per_100k": BQ_FLOAT}
    assert column_types == expected_column_types


class GcsToBqTest(TestCase):

    _test_data = [["label1", "label2", "label3"], ["value_a", "value_b", "value_c"], ["value_d", "value_e", "value_f"]]
    ...
    def testLoadValuesBlobAsDataframe(self):
        ...
        mock_attrs = {"download_as_string.return_value": json.dumps(self._test_data).encode("utf-8")}
        mock_blob = Mock(**mock_attrs)
        frame = gcs_to_bq_util.load_values_blob_as_df(mock_blob)

        self.assertListEqual(list(frame.columns.array), ["label1", "label2", "label3"])  # pylint: disable=E1101
        self.assertEqual(frame.size, 6)  # pylint: disable=E1101
    ...
    def testAddDataframeToBq_AutoSchema(self):
        ...
        with patch("ingestion.gcs_to_bq_util.bigquery.Client") as mock_client:
            ...
            gcs_to_bq_util.add_df_to_bq(test_frame.copy(deep=True), "test-dataset", "table")
    ...
    def testAddDataframeToBq_SpecifySchema(self):
        ...
        with patch("ingestion.gcs_to_bq_util.bigquery.Client") as mock_client:
            ...
            col_modes = {"label1": "REPEATED", "label2": "REQUIRED"}
            gcs_to_bq_util.add_df_to_bq(
                test_frame.copy(deep=True), "test-dataset", "table", column_types=column_types, col_modes=col_modes
            )
    ...
    def testLoadCsvAsDataFrame_ParseTypes(self, mock_bq: MagicMock):
        ...
        df = gcs_to_bq_util.load_csv_as_df("gcs_bucket", "test_file.csv", parse_dates=["col1"], thousands=",")
        ...
        expected_types = {"col1": np.dtype("datetime64[ns]"), "col2": np.int64, "col3": object, "col4": np.float64}
        ...
        # default object type.
        expected_types = {"col1": np.int64, "col2": object, "col3": object, "col4": object}
        for col in df.columns:
            self.assertEqual(df[col].dtype, expected_types[col])
