...
def test_set_state_col():
    ...
    assert_frame_equal(set_state_col(_fake_df), _expected_df_set_state_cols, check_like=True)
...
def test_filter_cols():
    ...
    assert_frame_equal(filter_cols(_fake_by_sex_df, std_col.SEX_COL), _expected_by_sex_filtered_cols, check_like=True)
...
def test_keep_only_states():
    assert (
        keep_only_states(_fake_by_race_df)
        .reset_index(drop=True)
        .equals(_expected_by_race_df_only_states.reset_index(drop=True))
    )
...
def test_keep_only_national():
    ...
    assert_frame_equal(
        keep_only_national(_fake_by_age_df_with_total, ["Female", "Male"]),
        _expected_by_age_df_only_national,
        check_like=True,
    )

    assert_frame_equal(
        keep_only_national(_fake_by_age_df_without_total, ["Female", "Male"]),
        _expected_by_age_df_only_national,
        check_like=True,
    )
...
def test_cols_to_rows():
    ...
    assert_frame_equal(
        cols_to_rows(_fake_bjs_table_df, ["Asian", "Black"], "race", "some_value"),
...
def test_swap_race_col_names_to_codes():
    ...
    assert_frame_equal(swap_race_col_names_to_codes(_fake_df), _expected_df_swapped_cols, check_like=True)
