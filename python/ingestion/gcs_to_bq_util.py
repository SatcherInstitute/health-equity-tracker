...
BQ_STRING = 'STRING'
BQ_FLOAT = 'FLOAT64'
API_KEY = 'TGSG5RI-3Z5U6JQ-SFBSMKA-6JPIG3A'
...
def __dataframe_to_bq(
    frame, dataset, table_name, column_types, col_modes, project, json_data, overwrite
):
...
def load_csv_as_df(
    gcs_bucket, filename, dtype=None, chunksize=None, parse_dates=False, thousands=None
):
...
def load_csv_as_df_from_web(
    url, dtype=None, params=None, encoding=None
) -> pd.DataFrame:
...
def load_xlsx_as_df_from_data_dir(
    ...
    sheet_df = pd.read_excel(
        file_path, sheet_name=sheet_name, dtype=dtype, na_values=na_values
    )
    return sheet_df
...
def load_json_as_df_from_data_dir_based_on_key_list(
    directory, filename, key_list
) -> pd.DataFrame:
    """Loads json data from /data/{directory}/{filename} into a DataFrame.
...
def load_public_dataset_from_bigquery_as_df(
    dataset, table_name, dtype=None
) -> pd.DataFrame:
