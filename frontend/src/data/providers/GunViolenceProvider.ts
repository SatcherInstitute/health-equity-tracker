...
export const POPULATION_METRICS: MetricId[] = ['fatal_population_pct', 'non_fatal_population_pct']
...
class GunViolenceProvider extends VariableProvider {
    ...
    getDatasetId(breakdowns: Breakdowns, dataTypeId?: DataTypeId, timeView?: string): DatasetId | undefined {
        if (timeView === 'current') {
            if (breakdowns.hasOnlyAge()) {
                if (breakdowns.geography === 'state') return 'cdc_wisqars_data-age_state_current'
                if (breakdowns.geography === 'national') return 'cdc_wisqars_data-age_national_current'
            }
            if (breakdowns.hasOnlyRace()) {
                if (breakdowns.geography === 'state') return 'cdc_wisqars_data-race_and_ethnicity_state_current'
                if (breakdowns.geography === 'national') return 'cdc_wisqars_data-race_and_ethnicity_national_current'
            }
            if (breakdowns.hasOnlySex()) {
                if (breakdowns.geography === 'state') return 'cdc_wisqars_data-sex_state_current'
                if (breakdowns.geography === 'national') return 'cdc_wisqars_data-sex_national_current'
            }
        }

        if (timeView === 'historical') {
            if (breakdowns.hasOnlyAge()) {
                if (breakdowns.geography === 'state') return 'cdc_wisqars_data-age_state_historical'
                if (breakdowns.geography === 'national') return 'cdc_wisqars_data-age_national_historical'
            }
            if (breakdowns.hasOnlyRace()) {
                if (breakdowns.geography === 'state') return 'cdc_wisqars_data-race_and_ethnicity_state_historical'
                if (breakdowns.geography === 'national') return 'cdc_wisqars_data-race_and_ethnicity_national_historical'
            }
            if (breakdowns.hasOnlySex()) {
                if (breakdowns.geography === 'state') return 'cdc_wisqars_data-sex_state_historical'
                if (breakdowns.geography === 'national') return 'cdc_wisqars_data-sex_national_historical'
            }
        }
    }
