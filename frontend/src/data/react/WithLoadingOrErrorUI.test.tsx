...
function WithMetricsWrapperApp(props: { query: MetricQuery; displayRow?: (row: Row) => void }) {
  return (
    <WithMetrics queries={[props.query]}>
      {([response]) => {
        return (
          <div data-testid='MetricQueryResponseReturned'>
            {response.dataIsMissing() && <>Error: {response.missingDataMessage!}</>}
            {!response.dataIsMissing() && (
              <>
                Loaded {response.data.length} rows.{' '}
                {props.displayRow !== undefined && response.data.map((row) => props.displayRow!(row))}
              </>
            )}
          </div>
        );
...
describe('WithLoadingOrErrorUI', () => {
  ...
  test('WithMetrics: Loads metrics', async () => {
    const query = new MetricQuery('copd_per_100k', Breakdowns.byState().andRace(excludeAll()));

    expect(dataFetcher.getNumGetMetadataCalls()).toBe(0);
    const { findByTestId } = render(
      <WithMetricsWrapperApp
        query={query}
        displayRow={(row: Row) => `${row.race_and_ethnicity}: ${row.copd_per_100k}. `}
      />
    );
    ...
    expect(element.textContent).toContain('Loaded 2 rows. AmIn: 20000. Asian: 1000.');
  ...
  test('WithMetrics: Loaded metrics have no rows', async () => {
    const query = new MetricQuery('depression_per_100k', Breakdowns.national().andRace());
  ...
  test('WithMetrics: Unsupported breakdown', async () => {
    const query = new MetricQuery('depression_per_100k', Breakdowns.byCounty().andAge());
    ...
    const element = await findByTestId('MetricQueryResponseReturned');
    expect(element).not.toBeNull(); // Equivalent to toBeInTheDocument
    expect(element.textContent).toContain('Error: Breakdowns not supported for provider ahr_provider: age:no filters,geography:county');
