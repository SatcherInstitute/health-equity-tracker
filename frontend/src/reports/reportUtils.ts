...
const ONLY_AGE_MAP: Partial<Record<string, DemographicType>> = {
  Age: 'age',
}

const ONLY_RACE_MAP: Partial<Record<string, DemographicType>> = {
  'Race/ethnicity': 'race_and_ethnicity',
}

const ONLY_SEX_RACE_MAP: Partial<Record<string, DemographicType>> = {
  'Race/ethnicity': 'race_and_ethnicity',
  Sex: 'sex',
}

const PHRMA_DEMOGRAPHIC_TYPES_MAP: Partial<Record<string, DemographicType>> = {
  'Race/ethnicity': 'race_and_ethnicity',
  Sex: 'sex',
  Age: 'age',
  Subsidy: 'lis',
...
export function getDemographicOptionsMap(
  ...
  if (dataTypeConfig1 && dataTypeConfig2) {
    if (getIsPhrma(dataTypeConfig1) && getIsPhrma(dataTypeConfig2))
      return PHRMA_DEMOGRAPHIC_TYPES_MAP
  }
