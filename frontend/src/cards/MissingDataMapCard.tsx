...
export interface MissingDataMapCardProps {
  key?: string;
  fips: Fips;
...
export function MissingDataMapCard(props: MissingDataMapCardProps) {
  return (
    <MissingDataMapCardWithKey
...
function MissingDataMapCardWithKey(props: MissingDataMapCardProps) {
  ...
  const queries = requestedBreakdowns.map(
    (breakdown) =>
      new MetricQuery(
        ...
        geographyBreakdown.copy().addBreakdown(
          breakdown,
          breakdown === "race_and_ethnicity"
            ? exclude(ALL, NON_HISPANIC, UNKNOWN, UNKNOWN_RACE) // EXCLUDE ALL HERE
            : undefined
        )
      )
  );
  ...
  return (
    <CardWrapper
      queries={queries}
      title={<>Missing Data for {metricConfig.fullCardTitleName}</>}
    >
