...
const CardLeftNumber: React.FC<CardLeftNumberProps> = ({
  ...
  return (
    <li className='fade-in-up-blur flex flex-row gap-4 my-10'>
      <div className='flex flex-col items-center justify-center w-1 max-w-1 gap-4 relative'>
        <span
          className={`text-[15rem] my-0 font-extrabold absolute top-[-2] left-0 m-0 p-0 text-hoverAltGreen ${spanColor}`}
        >
          {number}
        </span>
      </div>
      <div className='flex flex-col gap-2 ml-0 md:ml-4 h-full'>
        <div className='flex flex-col '>
          ...
          <p className='text-small w-fit py-0 my-0 text-altBlack h-full'>
            {description}
          </p>
        </div>
      </div>
