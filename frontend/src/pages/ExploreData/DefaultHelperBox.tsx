...
const reportMapping = [
  {
    ...
    description: 'Uncover disparities in HIV prevalence across different racial and ethnic groups in the U.S. Understanding these patterns is vital for targeted interventions and improved health equity.',
  ...
  },
  {
    ...
    description: 'Analyze COVID-19 mortality in Fulton County, GA, by age. Highlighting vulnerable populations helps to inform public health strategies and resource allocation.',
  ...
  },
  {
    ...
    description: 'Explore the intersection of incarceration, poverty, and race in Georgia. Addressing these disparities is key to improving health outcomes and social justice.',
    categories: ['Social Determinants of Health', 'Political Determinants of Health', 'Race/Ethnicity', 'State-Level', 'Compare Topics']
  },
  {
    setting: UNINSURANCE_SEX_FL_VS_CA_SETTING,
    title: 'Uninsurance in FL & CA by Sex',
    description: 'Examine uninsurance rates by sex in Florida and California. Identifying these gaps is crucial for advancing equitable healthcare access.',
    categories: ['Social Determinants of Health', 'State-Level', 'Sex', 'Compare Places']
  },
  {
    setting: PHRMA_HIV_ELIGIBILITY_USA_MULTIMAP_SETTING,
    title: 'HIV Disparity Maps by Medicare Eligibility',
    description: 'Visualize HIV disparities among Medicare beneficiaries. These insights are essential for optimizing treatment and reducing health inequities.',
    categories: ['HIV', 'Medication Utilization in the Medicare Population', 'Multiple Maps', 'National-Level'],
    icon: <FiberNewIcon />
  },
];
    ...
    <div className='flex w-full items-center justify-center bg-white px-12 pb-0 pt-4 sm:px-20 sm:pt-8'>
      <section className='m-0 mb-5 grid w-full max-w-helperBox grid-cols-1 content-center items-center justify-evenly justify-items-center rounded-md bg-standardInfo pb-0 smMd:grid-cols-5'>
        <div className='col-span-3 px-10 py-0 text-left smMd:px-0 md:px-10 xl:col-span-2'>
          <p className='text-smallerHeader text-altBlack font- leading-lhLoose'>Investigate rates of select a topic in the United States</p>
            ...
            {reportMapping.map((report, index) => (
              <li className='my-4' key={index}>
                ...
                >
                  ...
                  <h1 className="text-lg font-bold">{report.title}                     {report.icon && report.icon}</h1>
