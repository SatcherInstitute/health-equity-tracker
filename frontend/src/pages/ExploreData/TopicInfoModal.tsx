...
import { DATA_CATALOG_PAGE_LINK, TOPICS_LINK } from '../../utils/internalRoutes'
...
export default function TopicInfoModal() {
  ...
  const categoryId =
    useAtomValue(selectedDataTypeConfig1Atom)?.categoryId ?? 'behavioral-health'
  ...
  return (
    ...
    >
      ...
      <DialogContent dividers={true} sx={{ fontSize: sass.smallest }}>
        ...
        <HashLink to={`${TOPICS_LINK}/${categoryId}`}>methodology</HashLink>, or
        view the <HashLink to={DATA_CATALOG_PAGE_LINK}>source data</HashLink>.
      </DialogContent>
    </Dialog>
  )
}
