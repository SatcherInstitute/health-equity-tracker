...
`HetFeatureFlagIndicator` renders a 🧑🏽‍🔬 in both the desktop and mobile toolbars whenever **any** flag is on, from either source, so a flagged environment is never silently flagged. Hovering, or tapping on a touch device, opens an MUI tooltip naming every armed flag and its source, so finding out what is on does not require opening devtools; the same click prints a table of every flag, including any forced off. The tooltip is controlled rather than left to MUI's own listeners, since a touch device never fires hover, and it sets `describeChild` so the tooltip does not take over the button's accessible name while open.
