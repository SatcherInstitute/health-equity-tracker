...
test('Prison by Race', async ({ page }) => {
  ...
  await page.locator('#rate-chart').getByText('3,232 children confined in adult facilities in the United States. Learn more.').click();
  await page.locator('#unknown-demographic-map').getByRole('note').click();
  await page.getByRole('heading', { name: 'Breakdown summary for people in prison in the United States' }).click();
