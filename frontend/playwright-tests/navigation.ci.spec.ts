...
test('Clear selections button from Compare Topics mode returns tracker to default state', async ({
  ...
  const section = await page
    .getByRole('heading', { name: 'Prison & Poverty in Georgia by Race' })
    .locator('..')
...
test('Including the Extremes Mode Param in URL should load report with Extremes Mode Enabled', async ({
  ...
  await page
    .getByRole('heading', { name: '(only states/territories with' })
    .click()
  await page.getByRole('heading', { name: 'Highest:' }).click()
  await page.getByRole('heading', { name: 'Lowest:' }).click()
  await page
    .getByRole('button', { name: 'Reset to show all states/territories' })
    .click()
  await page
...
test('Extremes Mode Param in URL should work for both sides of Compare mode report', async ({
  ...
  await page
    .locator('#rate-map2')
    .getByRole('heading', { name: 'Ages 13+ (only counties with' })
    .click()
  await page
    .locator('#rate-map2')
