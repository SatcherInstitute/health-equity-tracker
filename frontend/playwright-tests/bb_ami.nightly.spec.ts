...
test('PHRMA: Beta Blockers after Heart Attack (AMI)', async ({ page }) => {
  ...
  await page.getByRole('heading', { name: 'Medicare Beta-Blocker Beneficiaries, Eligible due to disability, 18+' }).click();
  await page.getByLabel('Legend for rate map').getByRole('img').click();
  await page.locator('li').filter({ hasText: 'Total Medicare Beta-Blocker Beneficiaries, 18+ population:' }).click();
  ...
  await page.getByText('Population Receiving Persistent Beta Blocker Treatment After a Heart Attack', { exact: true }).click();
  await page.getByText('Measurement Definition: National Quality Forum measure representing the').click();
  await page.getByText('Clinical Importance: Beta-blockers are recommended by clinical guidelines for').click();
