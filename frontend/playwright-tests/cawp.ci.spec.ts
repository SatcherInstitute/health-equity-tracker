...
test('CAWP: Congress', async ({ page }) => {
  ...
  await page.waitForSelector('div[role="note"] >> text=No unknown values for race', { timeout: 120000 });
  ...
  await page.waitForSelector('text=Percentages reported for Women in state legislatures cannot be summed, as these', { timeout: 60000 });
...
test('CAWP: State Legislature', async ({ page }) => {
  ...
  await page.waitForSelector('#rate-map h3:has-text("Percentage of state")', { timeout: 60000 });
