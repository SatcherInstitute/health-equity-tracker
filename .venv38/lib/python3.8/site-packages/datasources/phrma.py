...
class PhrmaData(DataSource):
    ...
    def write_to_bq(self, dataset, gcs_bucket, **attrs):
        ...
        float_cols = [
            f'{std_col.MEDICARE_PREFIX}_{std_col.POPULATION_COL}_{
        ...
        for condition in PHRMA_PCT_CONDITIONS:
            ...
            float_cols.append(f'{condition}_{BENEFICIARIES}_{
                              std_col.RAW_SUFFIX}')
    ...
    def generate_breakdown_df(
        ...
        if demo_breakdown == std_col.RACE_OR_HISPANIC_COL:
            ...
            df = generate_pct_share_col_without_unknowns(
                df, count_to_share_map, cast(
                    PHRMA_BREAKDOWN_TYPE, demo_col), all_val
            )

        rename_col_map = {
            MEDICARE_POP_COUNT: f'{std_col.MEDICARE_PREFIX}_{
...
def load_phrma_df_from_data_dir(
    ...
    for condition in [*PHRMA_PCT_CONDITIONS, *PHRMA_100K_CONDITIONS]:
        if condition in PHRMA_PCT_CONDITIONS:
            condition_keep_cols = [*keep_cols,
                                   COUNT_YES, COUNT_TOTAL, ADHERENCE_RATE]
    ...
    df_merged = reduce(
        lambda df_a, df_b: pd.merge(
            df_a, df_b, on=merge_cols, how='outer'), topic_dfs
    )
